...
- `tcgplayer_matches` stores one current TCGPlayer lookup per product (URL, name, set, image, price, the raw price label such as Market, confidence, and `matched`, `needs_confirm`, `unmatched`, or `error`). `match_source` is `auto`, `confirmed`, or `manual`. `error_text` holds a failed lookup. It survives later Amazon jobs. Clear or re-run replaces an automatic match. A manual match stays until you clear it or set another URL. `tcgplayer_price_history` keeps each manual scrape’s price.
...
**Set TCGPlayer URL** on a product row scrapes that one product page and stores it as a manual match (name, set, product id, Market and Low or Listed price, image, and the time it was scraped). **Manual match** takes an Amazon product URL and a TCGPlayer product URL together: a new ASIN is added, an existing ASIN is refreshed and keeps its older Amazon prices, and the compare page opens. Manual matches are labeled Manual. Match on TCGPlayer and Re-match all leave them alone until you clear the match or paste a different URL. An invalid URL, a blocked page, or a page that does not show the name and price is reported in the UI and in `.catalog-desk/api.log`. Nothing is filled in from a guess.
...
A dry-run Amazon job matches against saved search HTML, so tests and fixture scrapes do not open TCGPlayer. Products that have only ever been seen in fixture jobs stay on that path. A product from a live Amazon scrape uses the Playwright session already open for that job and posts the same public search request the TCGPlayer results page posts (`mp-search-api.tcgplayer.com/v1/search/request`, no login). Lookups stay one product at a time, with the same delay as Amazon page turns. A JavaScript shell or any response that is not a results document is stored as an error, “TCGPlayer search returned no results page”, and is not called a miss. A block stops further requests. A per-row error keeps the matches already stored.
...
| POST | `/api/products/{id}/tcg-url` | Scrape one TCGPlayer product URL and store it as a manual match |
| POST | `/api/manual-match` | Scrape an Amazon product URL and a TCGPlayer product URL, link them, and return the product for compare |
